...
const showList = (arr) => {
    ...
    arr.forEach((item) => {
        ...
        <button onclick="deleteItem('${item.product_name}')">Delete</button>`;
        list.appendChild(li);
    });
};
...
btn.addEventListener('click', () => {
    function changeUrl() {
        const newUrl = `${window.location.pathname}?from=${fromPrice.value}&to=${toPrice.value}`;
        window.history.pushState({ path: newUrl }, '', newUrl);
    }
    changeUrl()
    let url = new URLSearchParams(window.location.search)
    console.log(parseInt(url.get('from')), parseInt(url.get('to')))
    let filteredArray = goods.filter((item) => item.product_price > parseInt(url.get('from')) && item.product_price < parseInt(url.get('to')))
...
search.addEventListener('input', () => {
    function changeUrl() {
        const newUrl = `${window.location.pathname}?from=${search.value}`;
        window.history.pushState({ path: newUrl }, '', newUrl);
    }
    changeUrl()
    let url = new URLSearchParams(window.location.search)
     console.log(url.get('from'))
    let filteredArray = goods.filter((item) => item.product_name.toUpperCase().startsWith(url.get('from').toUpperCase()))
    console.log(filteredArray)
    showList(filteredArray)
})
...
function deleteItem(productName) {
  goods = goods.filter(item => item.product_name !== productName);
  showList(goods);  }